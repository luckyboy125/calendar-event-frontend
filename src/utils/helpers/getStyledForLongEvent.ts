...
export const getStyledForLongEvent = (
  ...
  const lastDay = shmoment(lastItem.date)
    .add('hours', 23)
    .add('minutes', 59)
    .result();

  const isStartDayEvent = checkDateIsEqual(day.date, startDate);
  const isEventMovingToNextInterval =
    firtDayOfNextInterval.getTime() < endDate.getTime();
  const isEventMovingFromPrevInterval =
    checkDateIsEqual(firstItem.date, day.date) &&
  ...
  if (isStartDayEvent) {
    ...
    const diffInDays = getDifferenceOfTwoDates(
      day.date,
      isLastDate ? lastDay : endDate
    ).days;
