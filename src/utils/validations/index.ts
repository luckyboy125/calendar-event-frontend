export const validationMethodsWithoutParams = {
  ...
  isNumber(value: string) {
    ...
    return !regex.test(value)
      ? { status: false, errorMessage: 'The field must contains number' }
      : { status: true };
  },
  isNumberWithColon(value: string) {
    ...
    return !regex.test(value)
      ? {
          status: false,
          errorMessage: 'The field must contains number with colon',
        }
      : { status: true };
  },
...
export const validationMethodsWithParams = {
  ...
  min(value: string, min: number) {
    return +value < min
      ? { status: false, errorMessage: `The number cannot be less than ${min}` }
      : { status: true };
  },
  max(value: string, max: number) {
    return +value > max
      ? { status: false, errorMessage: `The number cannot be more than ${max}` }
      : { status: true };
  },
};
